      ...
      <Grid item xs={6}>
        <FormControl fullWidth>
          ...
          <Select style={{color:"white"}} value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })}>
            {selectedCategories.map((c) => <MenuItem style={{color:"white"}} key={c.type} value={c.type}>{c.type}</MenuItem>)}
          </Select>
        </FormControl>
      </Grid>

      <Grid style={{color:"white"}} item xs={6}>
        <TextField  type="number" InputLabelProps={{
          style: {
            color: 'white',
          } }}  label="Amount" value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} fullWidth />
      </Grid>
      <Grid style={{color:"white"}} item xs={6}>
        <TextField InputLabelProps={{
          style: {
            color: 'white',
          } }} fullWidth label="Date" type="date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: formatDate(e.target.value) })} />
      </Grid>
